import { memoizeLast } from '../memoize/memoize-last';
import { getT } from './t-tag';

function sameDate(a: Date | number, b: Date | number): boolean {
...
function sameYear(a: Date | number, b: Date | number): boolean {
	const dateA = new Date(a);
	const dateB = new Date(b);
