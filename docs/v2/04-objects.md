...
4. **Level 4 — Control / text container.** Leaf controls (`o-input-box` — buttons, inputs); standalone prose blocks just use `o-box` (it trims), or `o-prose` for multi-element flow. Reads its own `--o-*-radius` knob, which the enclosing box redefines.
...
|---|---|---|---|
| `o-box` | block + text | `--o-box__radius` | The universal padded container. Uniform block padding (`text-box: trim-both` makes it read optically uniform around raw text — a no-op for component children, so one box serves both modes), nesting-math inline padding (`--v-pad-inline`, the radius-cascade step, so a nested control stays concentric). Holds components/rows *or* raw text. Text that needs perimeter inset / alignment with sibling cards is `o-prose`'s job (§6.1, §9.5), not the box. |
...
| `o-code` | `--o-code__font-size` (default `var(--v-font-size-code)`) | Monospace text container for `<code>` and `<pre>`. Sets font-size, font-family (`--v-font-family-mono`), and a matching line-height. `<pre>` elements compose this with `o-box` for padded code blocks. Because `<pre>` carries its own padding, it gets `text-box-trim: trim-both` (the padded-box rule, §6) — its padding also absorbs the trimmed cap/descender overshoot, so horizontal `overflow` doesn't clip the first/last line. |

A single block of raw text just uses `o-box` (it trims for optical block padding); `o-prose` is what you reach for when the box holds **multiple** flowing text elements and you want the boundary-owned rhythm plus perimeter inline alignment with sibling cards.
...
| Class | Knobs | Purpose |
|---|---|---|
| `o-menu` | `--o-menu__pad-block`, `--o-menu__pad-inline`, `--o-menu__radius` (default `--v-radius-min`) | Popover frame for dropdowns, selects, command palettes, and similar floating list containers. Structural only — radius, padding, overflow, max-height. Chrome (shadow, border, backdrop) comes from the composing component or `v-surface-*`. Typically composes with `o-dialog` when used inside a `<dialog>` / popover. **Radius reads the floor, not the `--o-box__radius` cascade** — a popover floats over content rather than nesting in the frame's corner, so a small constant corner reads right regardless of the frame radius. **Block padding tracks `--o-menu__radius`** (not `--v-gap-block`): full-bleed rows highlight edge to edge, so the only block padding needed is enough to keep the first/last highlight inside the rounded corners — exactly the radius. |
