...
- [09-demo-site.md](./09-demo-site.md) — §19 Site architecture, §20 Pages (incl. §20.6, the three-tier customization story), §21 Theme workbench. **The site is the canonical documentation**, with a three-item top nav: **Composition** (§20.2 — `--v-*` variables, `o-*` objects, `t-*` tools, typography, under a sidebar/ToC), **Colors** (§20.3 — color system + contrast checker), **Components** (§20.4 — `c-*` catalogue). Built in that order, authored in Solid JSX and statically rendered; the Composition + Colors pages ship zero client JS and are built first as the CSS review artifact (see PLAN.md Phase 0–3).
...
- [spacing-workbench.html](./spacing-workbench.html) — Interactive rhythm/type/radius playground where the v2 spacing, fluid-type, list-rhythm, radius-cascade, inline-inset, full-bleed, and dialog (scroll-away header, sticky footer) decisions were worked out empirically. Live sliders drive a preview frame and a resolved-`:root` readout. Kept as a working reference for the choices folded into §6–§8 and §10.2; PLAN.md cites it inline. Open in a browser. *(Prototype — token names and a few demo strings predate the final spec; the .md docs are authoritative where they differ.)*
