...
- **Verify the fluid type scale** (§5.4): at a narrow viewport (≈`--v-viewport-min`) the computed `font-size` of `body`/`h1` sits near the `*-min` anchor; at a wide viewport (≈`--v-viewport-max`) it sits near the `*-max` anchor and is strictly larger; the heading→body *ratio* is larger at the wide viewport than the narrow one (dual-ratio behavior). Confirm no token's narrow-viewport size falls below the readability floor.
...
**Done when:** The **Objects page** renders: a padded `o-box` with nested `o-input-box` elements showing the preset-driven radii stepping *inward* from the frame (outer corners largest, inner ones bottoming out at the floor — never sharp); an `o-stack` of items; an `o-prose` block with boundary-owned heading/paragraph rhythm and an `<hgroup>`; an `o-grid` that reflows responsively; an `o-split` that collapses; an `o-segmented` settings group; and the `v-align-edge`/`v-align-text` modes. The four-level nesting model from §9.1 works visually and is reviewable on the page.
