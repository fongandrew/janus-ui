...
2. **`text-box.css`** — `o-text-box` with uniform block padding (it is itself a padded text box, so it carries `text-box: trim-both` per §6) and alignment-aware inline padding (§6.1). A perimeter + inline-alignment primitive; vertical rhythm between children comes from flow, not this box (§6.2).
