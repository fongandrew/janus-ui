...
1. **`box.css`** — `o-box` with `--o-box__pad-block`, `--o-box__pad-inline`; reads its own `--o-box__radius` with a `var(--v-radius-min)` fallback (no descendant redefinition — the radius cascade does that, §8).
