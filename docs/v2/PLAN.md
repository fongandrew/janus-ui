...
4. **`tokens/`** — root knobs and (internal) mixins. Mixins defined here are Janus-internal authoring tools, **not** a consumer API (§5.3): consumers theme via `--v-*` overrides, `v-`/`t-` classes, or their own CSS.
   - `spacing.css` — `--v-spacing`, the derived `--v-pad-block` / `--v-pad-inline` and the **named-multiple gap stack** `--v-gap-block` (×1, within-group), `--v-gap-inline` (×0.5, inline clusters), `--v-gap-tight` (×0.25, form label→control & settings rows), `--v-gap-section` (×1.5, between sections), `--v-control-inset` (§6.1), and the internal `v-spacing` mixin (§5.3, §6). The gap stack is the shipped, baked-in form of the workbench's `--rk-*` rhythm sliders (one master lever, named multiples — see `spacing-workbench.html`); a section-level `o-stack`/`o-grid` or a form field reaches for the matching gap token instead of hand-calcing a multiple. **The mixin should also bundle `--v-border-width`** — denser layouts want thinner borders, not just smaller padding. *(Aesthetic spec §13)* The mixin does **not** touch radius — radius is preset-driven and decoupled from spacing (§8), so a density change adjusts padding/gap/border but leaves corners alone. Also add the hi-DPI density bump — it moves both *font anchors* together (§5.4), not a frozen `--v-font-size`, keeping them collapsed under the default fixed config (so text stays fixed, just larger): `@media (resolution >= 200dpi) { :root { --v-input-height: 2.75rem; --v-font-size-min: 1rem; --v-font-size-max: 1rem; --v-spacing: 1rem; } }` — phone screens need larger touch targets and body text. *(Aesthetic spec §14)*
   - `radius.css` — the **max** `--v-radius` (the assumed window/frame corner — the anchor) and the **min** `--v-radius-min` (the floor), plus `--v-border-width` (§5.1, §8). Also the **always-on radius cascade** (no preset classes): the `:root` / `.o-box` / `.o-dialog` rules that assign the per-layer `--o-*__radius` by stepping **inward** from the max toward the min, floored (§8.2). The look is tuned by the two root values alone (flat = `min == max`). The frame radius drives the cascade whether or not it's painted.
   - `color.css` — `--v-bg`, `--v-fg` (with the OKLCH derivation), `--v-link`, `--v-accent`, `--v-muted`, `--v-danger-text`, the `--v-backdrop` scrim (the dialog/drawer backdrop fill, `light-dark()` so it darkens correctly in both schemes — components reference it, never a literal), weight-min knobs (§5.1, §7). **Additional tokens to add:**
     - `--v-border-dynamic-base: light-dark(black, white)`, `--v-border-dynamic-mix: 17.5%` (→ `50%` under dark, set by the color-scheme rules — **NOT** `light-dark(17.5%, 50%)`: `color-mix()` rejects `light-dark()` in the percentage slot and silently drops the whole border), `--v-border-color: color-mix(in hsl, var(--v-border-dynamic-base) var(--v-border-dynamic-mix), var(--v-bg))` — one recipe places every border at a fixed perceptual distance from any surface. *(Aesthetic spec §01)*
...
5. **`base.css`** — element-level styles. Layer: `base`. Consumes `--v-*` tokens. Beyond the basics (body, headings, links, form defaults, `code`, `pre`, tables), base.css carries several important v1 conventions:
   - **Text-box trim** follows *edges*, not element types (§6): apply `text-box: trim-both cap alphabetic` to a **single or padded** text box (a standalone block, `<pre>`, a `blockquote` callout, a table cell) so its perimeter padding reads uniform; apply `trim-start` / `trim-end` only to the **outer edges of a continuous group** (an `<hgroup>`, a continuous list), leaving members *between* untrimmed so their half-leading carries the rhythm. This lets text-mode block padding be uniform regardless of font size (no `(1lh − 1em)/2` compensation). Never trim inline text. Firefox lacks it (§15); accepted degradation is slightly loose first/last-line leading. *(see §6)*
   ...
   - **List indent**: `ul, ol { padding-inline-start: 1em }` — carried verbatim from the Slip Switch consumer fork. The `em` unit is the point: it **compounds per nesting level** so deep lists indent proportionally to their own (possibly smaller) text. Base also **cycles ordered-list markers by depth** (`decimal → lower-alpha → lower-roman → repeat`) — browsers cycle `ul` natively but never `ol`, so this evens the two up for any list. The *richer* marker treatment (hanging tabular-figure markers) lives in `o-prose` (Phase 2, folded from Slip Switch's editor CSS) and layers on top. *(workbench: `docs/v2/spacing-workbench.html`)*
   - **Form-control width**: base text inputs / `textarea` / `select` default to `inline-size: 100%` (fill their column — the app-form norm; `c-input` matches in Phase 3).
...
- Verify every primary knob (`--v-spacing`, `--v-radius`, `--v-border-width`, `--v-input-height`, `--v-font-family`, `--v-font-family-mono`, `--v-font-size-min`, `--v-font-size-max`, `--v-font-ratio-min`, `--v-font-ratio-max`, `--v-viewport-min`, `--v-viewport-max`, `--v-line-height`, `--v-bg`, `--v-link`, `--v-accent`, `--v-muted`, `--v-shadow-outer`, `--v-shadow-inner`) resolves to a non-empty concrete value via `getComputedStyle`.
- Verify secondary knobs (`--v-font-size` (the resolved fluid base), `--v-pad-block`, `--v-pad-inline`, `--v-gap-block`, `--v-gap-inline`, `--v-fg`, `--v-font-size-h1` through `-h6`, `-caption`, `-code`) all resolve to concrete `clamp()`/length values.
...
1. **`box.css`** — `o-box`, the universal padded box, with `--o-box__pad-block`, `--o-box__pad-inline`; reads its own `--o-box__radius` with a `var(--v-radius-min)` fallback (no descendant redefinition — the radius cascade does that, §8). Applies `text-box: trim-both` so block padding reads optically uniform around raw text (a no-op for component children) — this folds in the former `o-text-box`, so there is no separate text-box object. Inline padding stays `--v-pad-inline` (the cascade step, so a nested control is concentric); perimeter text alignment is `o-prose`'s job (§6.1, §9.5), and centered control text keeps its own radius floor in `o-input-box` (§8.6).
2. **`o-text-box` — folded into `o-box`.** There is no `text-box.css`: the text-mode block-padding trim is now part of `o-box` (item 1), which serves both component and raw-text contents. Raw text that needs perimeter inset / cross-card alignment uses `o-prose` (§9.5).
3. **`input-box.css`** — `o-input-box` with `--v-input-height`, text-mode padding (inline padding floored at the corner radius, `max(--v-control-inset, --o-input-box__radius)`, §6.1 / §8.6). Reads its own `--o-input-box__radius` with fallback to `--v-radius-min` (§8). Shared base for buttons, inputs, textareas, native selects.
...
16a. **`prose.css`** — `o-prose` (§9.5, new): block-flow running-text container with **boundary-owned** vertical rhythm (margin on the lower element; body↔body & heading→content = the prose gap; content→heading = space-above) and which opts its immediate text children into `text-box-trim`. Plus the **`<hgroup>`** base styling — additive leading (`--o-hgroup__leading`) and outer-edge-only trim (`trim-start`/`trim-end`) so title+subtitle read continuous (§6.2). The prose gap is line-based, not `--v-spacing`-based.
...
Build the **Objects sub-page** (§20.2.2, under Composition) into the site — one demo card per object (including `o-bar` and `o-segmented`), each showing a live render (neutral placeholder content so structure is the focus), the object's `--o-<name>__*` knobs with defaults, and a copyable markup snippet. Must render: the four-level nesting (`o-dialog` > `o-box` > `o-input-box`) at a couple of `--v-radius` / `--v-radius-min` settings (§8) so the stepped (large range) vs. flat (`min == max`) corners are visible and the computed radius is annotated at each tier; the inline text-inset modes side by side (§6.1) — a form field's label/description and a prose block at `v-align-edge` (flush), the straight-edge default, and `v-align-text` (inner-text line), with a code block breaking out of the prose; `o-grid` / `o-split` reflow at multiple widths; and an `o-segmented` settings group. Add it to the Composition sidebar/ToC. This is the review artifact for the object layer and the E2E target. (Now that `o-box` / `o-grid` etc. exist, also use them to firm up the ad-hoc site shell.)
...
- **Radius cascade** (§8): render `o-dialog` > `o-box` > `o-input-box` and verify radius steps **inward** — frame = `--v-radius` (max), then each level = `max(--v-radius-min, parent − pad/inset)`, so dialog > box > control and a control nested in a box rounds *less* than one sitting directly in the dialog (depth, not type). Confirm no layer falls below `--v-radius-min` (never sharp), and that `dialog radius = window − --o-dialog__offset`. Set `--v-radius-min == --v-radius` and verify every corner is identical (flat).
...
- **Spacing**: verify `o-box` padding matches `--v-pad-block` / `--v-pad-inline` in component mode, and that its block padding stays uniform `--v-pad-block` around raw text (text-box-trim handles optical alignment — no lh-compensation subtraction).
...
The narrow set from §11: `t-px-0`, `t-py-0`, `t-p-0`, `t-px`, `t-py`, `t-p`, `t-flex`, `t-flex-fill`, `t-flex-auto`, `t-flex-none`, `t-flex-wrap`, `t-items-start`, `t-items-center`, `t-items-end`, `t-justify-start`, `t-justify-center`, `t-justify-end`, `t-justify-between`, `t-block`, `t-inline`, `t-inline-block`, `t-hidden`, `t-sr-only`, `t-border`, `t-border-none`, `t-border-inner`, `t-radius-none`, `t-radius-full`, `t-shadow`, `t-shadow-inner`, `t-shadow-outer`, `t-shadow-none`, `t-text-start`, `t-text-center`, `t-text-end`, `t-truncate`.
...
> **Shell flag (carried from Phase 0).** The doc-site page shell currently wraps content in `p-page` (a plain `max-inline-size` + `margin-inline: auto` div, see `src/v2-site/site.css`), which is *not* a query container. For the full-bleed `@container frame` rule above to actually fire on the doc pages — and for a card / box full-bleed demo to be reviewable — the content frame needs to be an **`o-container`** (`container-type: inline-size; container-name: frame`). Swap `p-page`'s inner column to `o-container` (or compose the two) when wiring full-bleed here; the broader shell→Solid-component refactor is still Phase 9.
...
## Phase 3.5: Token remediation — reconcile with the spacing workbench

**Why:** `docs/v2/spacing-workbench.html` is the empirically-tuned reference for the whole rhythm/type/radius/dialog system, and it reads as near-final. Every **slider** in it represents a knob a consumer should be able to drive — so every slider must correspond to a real v2 variable (the *names* and any workbench-private intermediates need not match; the tunable *dimension* must exist). An audit of the workbench sliders against the shipped `src/lib2/css` tokens found the variable set substantially complete but with three gaps. This phase closes them so the shipped surface mirrors the workbench's tunable surface.

**Dependency:** Phase 1 (tokens), Phase 2 (objects), Phase 3 (components consume the new tokens).

**Audit result** — every workbench slider maps to a shipped variable *except*:

| Workbench slider | Drives | Shipped status |
|---|---|---|
| ① Section ×spacing | section grid/stack gap | **gap** — only an ad-hoc inline `calc(--v-spacing * 1.5)`; no named token |
| ④ Tight ×spacing | form label→control & settings-row gap | **gap** — absent entirely |
| (no slider; `:root` token) | dialog/drawer backdrop scrim | **gap** — `--v-backdrop` absent; `c-modal`/`c-drawer` hard-code `hsl(0 0% 0% / 50%)` |

All other sliders already have a variable: master/control-height (`--v-spacing` / `--v-input-height`), pad (`--v-pad-*`), cluster (`--v-gap-inline`), prose (`--o-prose__gap`), heading-space-above (`--o-prose__space-above`), hgroup-leading (`--o-hgroup__leading`), list rhythm (`--v-list-rhythm`), form-label inset (`--o-field__inset`), prose inset (`--o-prose__inset`), radius/min (`--v-radius` / `--v-radius-min`), dialog offset (`--o-dialog__offset`), and the full font block (`--v-font-size-min/max`, `--v-font-ratio-min/max`, `--v-viewport-min/max`, `--v-line-height`). *(Default-value differences — e.g. the workbench's pad ×1.25 vs the shipped ×1.0, prose ×1.0 vs ×0.75 — are intentional tuning choices, not missing variables, and are left as-is unless a later pass re-tunes them.)*

**Work:**

1. **`tokens/spacing.css`** — extend the `v-spacing` mixin with the named-multiple gap stack so they ride the same master lever (and the density bundle, §6.4): add `--v-gap-tight: calc($(size) * 0.25)` and `--v-gap-section: calc($(size) * 1.5)` alongside the existing `--v-gap-block` / `--v-gap-inline`. These are the baked form of the workbench `--rk-tight` / `--rk-section` sliders.
2. **`objects/stack.css`, `objects/grid.css`** — add the rhythm modifiers a layout reaches for instead of hand-calcing: `o-stack--tight` (`--o-stack__gap: var(--v-gap-tight)`), `o-stack--section` (`--o-stack__gap: var(--v-gap-section)`), `o-grid--section` (`--o-grid__gap-block: var(--v-gap-section)`). (The Phase 6 `LabelledInput` field stack uses `o-stack--tight` for its label→control rhythm.)
3. **`tokens/color.css`** — add `--v-backdrop: light-dark(hsl(216deg 16% 8% / 50%), hsl(216deg 16% 2% / 60%))`; point `c-modal::backdrop` / `c-drawer::backdrop` at it.
4. **Docs** — surface the new tokens on the Variables sub-page (spacing + color sections) and reconcile §6.2: the section gap is now a **named multiple** (`--v-gap-section`), not an ad-hoc bump — "one master lever, named multiples" (the spacing-workbench framing) rather than "no token at all."

**Done when:** every slider in `spacing-workbench.html` has a corresponding shipped variable; `o-stack--tight` / `--section` and `--v-backdrop` resolve to concrete values; the Variables page documents them; lint/build/E2E green.
...
1. **Config modal** (§21): carry forward existing prefs (color scheme, animations, font family, font size). Add the **Knobs section** — text inputs for all primary knobs (`--v-spacing`, `--v-radius`, `--v-input-height`, `--v-border-width`, the fluid type anchors/ratios `--v-font-size-min`, `--v-font-size-max`, `--v-font-ratio-min`, `--v-font-ratio-max`, `--v-viewport-min`, `--v-viewport-max`, `--v-line-height`, `--v-font-family`, `--v-font-family-mono`), secondary knobs behind a `<details>` expando (the resolved `--v-font-size`, pad/gap derivatives, font-size/line-height derivatives, shadow knobs). Editing the type anchors/ratios visibly reshapes the fluid scale (resize the window to see the interpolation). Persist via `ui-style-prefs`. Reset button clears all. This is the live-editing counterpart to the static **Variables sub-page** (§20.2.1) — changes made here visibly affect that page (and every other).
...
5. **Composition** (§20.2 — Variables, Objects, Tools, Typography) — already built and framework-free (Phases 1–3). In this phase: refactor the ad-hoc shell chrome (sidebar, cards) into real Solid components, ensure cross-links from Home, and confirm the config modal's live knobs visibly drive the Variables sub-page. Add the per-token **scoped slider islands** to the Variables sub-page and the **width-draggable type frame** to Typography (§20.2.1, §20.2.4): each is sliders driving a *scoped* preview region (`@scope` or a small `<iframe>`, not the page) + a resolved-token readout + a copy-CSS button. These are **distinct from the config modal** (item 1) and do the opposite job — local/ephemeral teaching tools (sliders, scoped) vs. the modal's global/persistent editor (text fields for `calc()`/`var()`, writes to `<html>` + localStorage); keep both, don't unify. The prototype `docs/v2/spacing-workbench.html` (controls → live preview → resolved `:root`, plus its draggable-width frame for fluid type) is the reference implementation. Also make the **Objects radius-cascade demo** (§20.2.2) interactive here: its per-level labels currently carry *static* rem strings hand-matched to a `--v-radius` override; replace them with `--v-radius` / `--v-radius-min` sliders driving the surfaces and each label showing its element's **computed** `border-radius` live, so the demo tracks the real shipped anchor instead of fake numbers.
