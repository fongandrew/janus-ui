...
- **The site is the documentation.** The shipped site (§19–21) is the canonical reference, not a separate marketing page. It documents every public surface — the `--v-*` variable knobs, the `o-*` objects, the `t-*` tools, and the `c-*` components — with live, rendered examples alongside the values. The site is **authored in Solid JSX from day one** and rendered to static HTML at build time (SSG); the CSS-layer documentation pages (variables, objects, tools, typography, colors, base elements) ship with **zero client JS** — their built output is plain HTML + `index.css`. That output is the human-review artifact while the CSS is iterated, the E2E target for the CSS layer, and the proof of the CSS-first claim: the *rendered* docs for the CSS package need nothing but the CSS package. (Authoring in JSX vs. hand-written HTML is a build-time choice that doesn't weaken the claim — and it's how the "component applies the right classes" pattern, §4.1, gets exercised from the first page. The library itself stays framework-agnostic; the *site* commits to Solid.)
