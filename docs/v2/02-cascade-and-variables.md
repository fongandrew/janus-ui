...
- `--v-border-dynamic-base` (default `light-dark(black, white)`), `--v-border-dynamic-mix` (default `17.5%`, set to `50%` under dark by the color-scheme rules), and the resolved `--v-border-color: color-mix(in hsl, var(--v-border-dynamic-base) var(--v-border-dynamic-mix), var(--v-bg))` — one recipe places every border a fixed perceptual distance from any surface, so borders read correctly on tinted cards/alerts without hard-coding a gray. **Note:** the *base* can be a `light-dark()` color, but the *mix percentage must be a literal* — `color-mix()` rejects `light-dark()` in the percentage slot (it invalidates the whole declaration and silently drops the border), so the mix is switched per scheme via the color-scheme rules, not `light-dark(17.5%, 50%)`. This keeps both knobs reusable as a recipe against other surfaces (e.g. `c-tabs` mixes the base into `--v-accent`).
