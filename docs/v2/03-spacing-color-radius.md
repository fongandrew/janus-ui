...
**The gap stack is a set of named multiples of `--v-spacing`.** Rather than one base gap plus ad-hoc per-context calcs, the box-rhythm gaps are **named multiples off the master lever** (the shipped form of the spacing workbench's `--rk-*` sliders — "one master lever, named multiples"): `--v-gap-block` (×1, within a group), `--v-gap-inline` (×0.5, inline clusters), `--v-gap-tight` (×0.25, a form's label→control and settings-row rhythm), and `--v-gap-section` (×1.5, *between* page sections / card groups — sections should breathe more than their contents). They all ride `--v-spacing` and the density bundle, so a density change moves the whole stack together. A layout reaches for the matching token by *naming* it — `o-stack--tight` / `o-stack--section` set `--o-stack__gap`, `o-grid--section` sets `--o-grid__gap-block` — instead of hand-calcing a fraction. (Text rhythm is still separate: it scales with the line, not `--v-spacing` — see above.)
...
- **One level of box-in-box.** A box nested directly inside another box paints at the **control radius** (one step in from its parent), so the inner box reads concentric rather than echoing the parent's corner: `:where(.o-box) .o-box { border-radius: var(--o-input-box__radius) }`. This overrides the *painted* corner only — rewriting `--o-box__radius` from `--o-input-box__radius` would be a self-referential cycle (the read-one/write-another rule, §8.2). Deeper nesting stays at the control radius (floored at the min); for more than one genuinely distinct rounded-box level, introduce a new object with its own knob.
