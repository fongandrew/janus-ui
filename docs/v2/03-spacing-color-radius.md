...
**Full-bleed is a per-frame opt-in — the `v-bleed` variant.** The mechanism ships always, but it fires only on frames carrying `v-bleed` (one class, valid on any frame — an `o-container` page region or an `o-dialog`; one shared `@container` rule serves both). The workbench treats full-bleed as a mode, and blanket-breaking every card out of every narrow frame is too opinionated for a default. It's a `v-` class because that's exactly what the variants layer is for (§4): a scoped re-mode of the library classes in a subtree — `v-bleed` has no look of its own; it changes how the boxes, grid, and prose beneath it respond to a narrow frame. A **class**, not a variable or attribute, on purpose: it's a structural mode chosen at authoring time, while `data-v-*` attributes are reserved for *runtime state* (`data-v-color-scheme`), and a custom property can't do the job at all (`@container` conditions can't read custom properties, and gating the rule body on one would need `style()` container queries, which aren't dependable at our §15 browser floor). The breakpoint is likewise a fixed length in the `@container` condition (`30rem`) for the same reason — changing it is a consumer-CSS override, not a knob.
