...
| Class | Element | Notes |
|---|---|---|
| `c-button` | `<button class="o-input-box">`, `<a class="o-input-box">` | Hover state, outer shadow (`--v-shadow-outer`), tone via `v-colors-*`. Layers chrome on `o-input-box`; sizing/padding/radius come from the object (inline padding `var(--v-spacing)` — roomier than inputs, §9.2). May be a flex container for icon+label centering — buttons are deliberately **not** on the `text-box-trim` list and never depend on trim; their line centers via `o-input-box`'s height math (§6). `c-button--icon` opts into square mode; a circular icon button (`border-radius: 50%`, `--v-input-height` square) is the frame-corner inhabitant (§8.5). Consumer-defined scopes (e.g. `.v-cta`) tweak height/radius for specific roles. |
| `c-card` | `<article>`, `<section>` (with `o-box`) | Surface chrome only (`--v-shadow-outer`, border, bg). Always pairs with `o-box`; prose content goes in an `o-prose` child (§6 — there is no `o-text-box`). Reads `--o-box__radius`. **Context-aware chrome** (all container-query-driven, so it tracks the card's *own* available width, not the viewport): cards inside `o-container` on wide viewports lose their shadow (they're in the reading flow); below a narrow threshold a card can **collapse to a full-bleed section** — `--c-card__radius`, inline border, and shadow drop to `0`/none and `margin-inline` breaks out by `var(--o-container__gutter)` (the sanctioned negative-margin case, §6.1). This is the *same* full-bleed mechanism a dialog's boxes use when the **dialog** is narrow — one `@container` rule keyed to the shared `frame` container (§9.3 `o-container`), so the surrounding prose realigns to flush by the same trigger (§6.1) — and it is **opt-in per frame** via the `--bleed` frame modifier (`o-container--bleed` / `o-dialog--bleed`, §6.1). `c-card` also sets `--o-stack__gap: var(--v-pad-block)` for a direct-child stack, so a card's internal rhythm defaults to its own padding (§6.2). Cards inside `:modal` / `:popover-open` always show their shadow (they're floating). For a **grouped/segmented** set of cells sharing one border (settings groups), compose `o-segmented` (§9.8) under the card surface. The `:has(:invalid, [aria-invalid]) [type="submit"]` rule visually disables the submit at `opacity: 0.5` but preserves `pointer-events` so the form engine can dispatch focus to invalid fields on click. |
...
| `c-table` | `<table>` | Data table. Row height via `--c-table__row-height` (see below). Row dividers via `--v-border-color`; header cells at `--v-font-weight-label`. |

**`c-table` — control-height rows, decoupled from controls.** v1's data-table cells borrowed the input-box padding math and it looked right — a row of cells reads like a row of controls, so tables and forms share one vertical rhythm. v2 keeps the *look* but cuts the coupling: `--c-table__row-height` **defaults to `var(--v-input-height)`** (parity out of the box) but is its own knob, so a dense data table can drop its rows without shrinking every form control on the page — and a touch-target bump to `--v-input-height` doesn't bloat data tables unless you want it to. It's a `--c-*` knob because there is no object behind it (§5).

Cell metrics follow the control recipe, not the trim recipe: block padding `calc((var(--c-table__row-height) − 1lh) / 2)` (centers a single line; multi-line content just grows the row past the minimum), inline padding `var(--v-control-inset)`. This is deliberate (§6): controls center by **height math**, which behaves identically whether or not the browser supports `text-box-trim` — if cells trimmed but inputs didn't (or vice versa), a table with an inline-editable cell would misalign by the half-leading in exactly one browser. Cells are padded text boxes, so they *additionally* take `trim-both` where supported (the §6 perimeter rule) — a pure optical refinement on the padding, never the thing that sets the height.

**Implicit text-mode.** Components that wrap text rely on the underlying text-mode object — `c-button`/`c-input` via `o-input-box`, `c-tag`/`c-badge`/`c-alert` via their own text-mode padding. The consumer never wraps them in anything. Bare prose gets correct optical block spacing from `text-box-trim` applied by its container (`o-prose` / `o-stack`, §6), not per-element compensation. A consumer reaches for `o-prose` explicitly only when writing running text that isn't already inside a text-bearing component (e.g., a callout card that *is* a paragraph — `o-box > o-prose`).
...
| `c-modal` | `<dialog class="o-dialog">` | Layers modal-typical chrome on `o-dialog`: centered viewport positioning, backdrop (`var(--v-backdrop)` tint + `backdrop-filter: blur(4px)`), focus halo, header/body/footer slot conventions. Width via `--c-modal__width` — comfortable default `40rem`, plus named presets: container-width (`var(--o-container__max)`) and narrow (`22rem`). `requestClose` for ESC / outside-click / `commandfor close` hooks. Focus trapping is native to `<dialog>`. Scrollable content areas show `--v-shadow-inner-top` / `--v-shadow-inner-bottom` at scroll edges (driven by `data-scroll-top` / `data-scroll-bottom` sentinels from `t-scroll-shadow.ts`). |
...
**Footer stickiness — opt-in, three modes.** `none` (footer scrolls in flow — the default for a bare `c-modal`); `submit` (only the primary `[type="submit"]` pins bottom-right via a sticky float mirror — its in-footer slot goes `visibility: hidden`, secondary actions scroll away; stickiness is invisible until the dialog actually overflows); `bar` (the whole footer pins as a glass action bar that breaks out to the dialog edges with a top border + scroll-edge shadow, rounding its own bottom corners). Per-button stickiness was considered and rejected — it reads badly with anything beyond the submit. The pinned submit composes with the `:has(:invalid) [type=submit]` disabled-but-clickable treatment (§10.1). **`ModalForm` (§13.6) defaults to `submit`** — a form's primary action should stay reachable while its fields scroll (the workbench's default demo state); it's opt-outable like its other behaviors.
...
- Box alignment (flex/grid `align-items` on the element it's applied to): `t-align-start`, `t-align-center`, `t-align-end`
- Text alignment (`text-align`): `t-text-start`, `t-text-center`, `t-text-end` — a **separate trio** from the box-alignment tools. They answer different questions ("where do my flex children sit on the cross axis" vs. "where do my inline boxes justify") and conflating them in one class forces the tool to guess from context. One element can legitimately carry one of each.
